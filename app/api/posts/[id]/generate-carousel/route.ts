...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const prompts = Array.isArray(body?.prompts)
    ? body.prompts
        ...
        .map((p: string) => p.trim().slice(0, MAX_PROMPT_LEN))
    : [];

  if (prompts.length !== CAROUSEL_SLIDE_COUNT) {
    return NextResponse.json(
      { error: "bad_request", message: `Exactly ${CAROUSEL_SLIDE_COUNT} slide prompts are required` },
      { status: 400 },
    );
  }
  ...
  try {
    const images = await Promise.all(prompts.map((prompt: string) => generateImage(prompt)));

    const imageUrls = await Promise.all(
      images.map(async (image, index) => {
        const path = `${user.id}/${id}-carousel-${index}.png`;
        const { error: uploadError } = await supabase.storage
          .from("post-images")
          .upload(path, Buffer.from(image.base64, "base64"), { contentType: "image/png", upsert: true });
        if (uploadError) throw new Error("Failed to upload carousel image");
        const publicUrl = supabase.storage.from("post-images").getPublicUrl(path).data.publicUrl;
        return `${publicUrl}?v=${Date.now()}`;
      }),
    );

    const tokensUsed = images.reduce((sum, image) => sum + (image.tokensUsed ?? 0), 0);

    const { data: updatedAnalysis, error: updateError } = await supabase
      .from("analyses")
      .update({
        carousel_prompts: prompts,
        carousel_image_urls: imageUrls,
        carousel_tokens_used: tokensUsed,
      })
      .eq("id", analysis.id)
      .select()
      .single();
    ...
    if (updateError || !updatedAnalysis) {
      throw new Error("Failed to save generated carousel");
    }
